...
class TestOktaRateLimiter:
    ...
    async def test_circuit_breaker_recovery(self) -> None:
        ...
        mock_func = AsyncMock()
        mock_func.side_effect = [
            RuntimeError("Initial failure"),  # Opens circuit breaker
    ...
    async def test_rate_limit_enforcement(self) -> None:
        ...
        # Execute requests that should trigger rate limiting
        await rate_limiter.execute_with_retry(mock_func)
        await rate_limiter.execute_with_retry(mock_func)
        await rate_limiter.execute_with_retry(mock_func)
    ...
    async def test_retry_after_header(self, rate_limiter: OktaRateLimiter) -> None:
        ...
        mock_func = AsyncMock()
        mock_func.side_effect = [
            RateLimitExceeded("Rate limited", retry_after=0.1),
            "success",
        ]
    ...
    async def test_concurrent_rate_limiting(
        self, rate_limiter: OktaRateLimiter
    ) -> None:
